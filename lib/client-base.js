...
class ClientBase extends EventEmitter {
  ...
  [kEnqueue] (opts, handler) {
    ...
    try {
      ...
      const request = new Request(opts, this[kUrl].hostname, handler)
      ...
      process.nextTick(handler.onError.bind(handler), err, handler)
    }
  }
...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnMessageComplete] () {
    ...
    if (this.statusCode >=  200 && !request.destroyed) {
