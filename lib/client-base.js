...
class ClientBase extends EventEmitter {
  ...
  [kEnqueue] (opts, handler) {
    if (!opts || typeof opts !== 'object') {
      process.nextTick(handler.onError, new InvalidArgumentError('invalid opts'))
      return
    }

    if (this[kDestroyed]) {
      process.nextTick(handler.onError, new ClientDestroyedError())
      return
    }

    if (this[kClosed]) {
      process.nextTick(handler.onError, new ClientClosedError())
      return
    }
    ...
    let request
    try {
      request = new Request(opts, this[kUrl].hostname, handler)
    } catch (err) {
      process.nextTick(handler.onError, err, handler)
      return
    }
...
class Parser extends HTTPParser {
  ...
  [HTTPParser.kOnHeadersComplete] (versionMajor, versionMinor, headers, method,
    ...
    request.headers(statusCode, headers, this.resumeSocket)
  ...
  [HTTPParser.kOnBody] (chunk, offset, length) {
    ...
    if (ret == null && request.read > this.client[kMaxAbortedPayload]) {
  ...
  [HTTPParser.kOnMessageComplete] () {
    ...
    if (this.statusCode > 100 && !request.destroyed) {
